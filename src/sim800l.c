...
void resetSim800l()
{
	gpio_set_direction(MODEM_RST,UART_MODE_UART);
	...
	if(res)
	{
		ESP_LOGW(TAGZ,"SIM800L GPRS is deactivated");
	}	
	res = runSingleGSMCommand(&cmd_deactBearer);
	if(res)
	{
		ESP_LOGW(TAGZ,"SIM800L Bearer profile is deactivated");
	}
	
	res = runSingleGSMCommand(&cmd_terminateHTTP);
	if(res)
	{
		ESP_LOGW(TAGZ,"SIM800L HTTP Session is deactivated");
	}
	...
	ESP_LOGI(TAGZ,"SIM800L is reset!");
}
...
static void infoCommand(char *cmd, int cmdSize, char *info)
{
	...
	ESP_LOGI(TAGZ,"%s [%s]", info, buf);
}

void configureUART()
{
	gpio_set_direction(MODEM_TX, UART_MODE_UART);
	gpio_set_direction(MODEM_RX, GPIO_MODE_INPUT);
	gpio_set_pull_mode(MODEM_RX, GPIO_PULLUP_ONLY);
    ...
    uart_set_pin(uart_num, MODEM_TX, MODEM_RX, UART_PIN_NO_CHANGE, MODEM_POWER_ON);
	...
	while(1)
	{
		...
		else 
		{
			if (tot > 0) {
				...
				{
					#if GSM_DEBUG
					ESP_LOGI(TAGZ,"AT RESPONSE: [%s]", sresp);
					#endif
					break;
				}
				else 
				{
					if (resp1 != NULL) 
					{
						...
						{
							#if GSM_DEBUG
							ESP_LOGI(TAGZ,"AT RESPONSE (1): [%s]", sresp);
							#endif
							res = 2;
							break;
						}
					...
					// no match
					#if GSM_DEBUG
					ESP_LOGI(TAGZ,"AT BAD RESPONSE: ---> [%s]", sresp);
					#endif
					res = 0;
					break;
				}
...
{
	#if GSM_DEBUG
		ESP_LOGI(TAGZ,"Initialization starting...");
	#endif
	...
	for(int gsmCmdIter = 0; gsmCmdIter < CmdsSize; gsmCmdIter++)
	{
		...
		{
			#if GSM_DEBUG
			ESP_LOGW(TAGZ,"Wrong response, retrying...");
			#endif
			...
			{
				#if GSM_DEBUG
				ESP_LOGE(TAGZ,"Initialization failed after 10 tries...");
				#endif
				resetSim800l();
				return 0;
				break;
